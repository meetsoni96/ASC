...
(
 ...
 Match_Title NVARCHAR(500) NOT NULL,
 Match_MPN NVARCHAR(200),
...
@Match_Title NVARCHAR(500),
@Match_MPN NVARCHAR(200) = NULL

AS
BEGIN

SET NOCOUNT ON;	
INSERT INTO dbo.Products(Category_ID,Domain_ID,Title,Price,SKU,Strike_Through_Price,[Availability],MPN,Product_Url,Image_Url,Match_Title,Match_MPN,DateTime_Inserted,DateTime_Last_Crawled)
VALUES(@Category_ID,@Domain_ID,@Title,@Price,@SKU,@Strike_Through_Price,@Availability,@MPN,@Product_Url,@Image_Url,@Match_Title,@Match_MPN,GETDATE(),GETDATE())
...
IF EXISTS(SELECT 1 FROM sys.procedures 
          WHERE Name = 'GetProductsByMPNOrTitle')
BEGIN
	DROP PROCEDURE dbo.GetProductsByMPNOrTitle
END
...
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[GetProductsByMPNOrTitle]
...
IF NOT EXISTS (
 ...
 WHERE  object_id = OBJECT_ID(N'[dbo].[Products]') 
        AND name = 'Match_Title'
)
BEGIN

ALTER TABLE Products
ADD  Match_Title NVARCHAR(500) NOT NULL,
 Match_MPN NVARCHAR(200) NULL
